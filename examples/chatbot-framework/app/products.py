...
def product(row,text,level):
    prompt=generate_prompt(text,"Don't use Where clause at all, Use Order_by command to order the rating OR price in Descending order and list top 4 items")
...
def other_products(row,text,level):
    prompt=generate_prompt(text,"Use Order_by command to order the rating in Ascending order and list top 4 items")
    response,response_token_product = call_text_completion(prompt)
...
def cheap_products(row,user_input,query_to_db,level):
    if "Product" in query_to_db:
        prompt=generate_prompt(user_input,"Don't use Where clause, Use Order_by command to order the price in Ascending order and list top 4 items")
    else:
        prompt=generate_prompt(user_input,"Use Order_by command to order the price in Ascending order and list top 4 items")
    response,response_token_product = call_text_completion(prompt)
...
def general_product(row,user_input,query_to_db,level): 
    ...
    prompt=generate_prompt(query_to_db,"Use Order_by command to order the rating in Descending order and list top 4 items")
    response,response_token_product = call_text_completion(prompt)
    output = execute_query(prompt,row,response,level,general_product.__name__)
    if output == []:
        prompt=generate_prompt(user_input,"Use Order_by command to order the rating in Descending order and list top 4 items")
    response,response_token_product = call_text_completion(prompt)
    output = execute_query(prompt,row,response,level,general_product.__name__)
    return(output, response_token_product)
