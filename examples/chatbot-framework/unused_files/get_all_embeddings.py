...
def embedding():
    ...
    if (df2_length == len(df1)):
        ...
        if (df2_length != 0):
            print(
                f"Embedding completed for {df2_length} rows. So, let's skip {df2_length} rows")
            df1 = df1[df2_length:]
        else:
        ...
        for index, chunk in enumerate(chunks, start=1):
            chunk['embedding'] = chunk['prompt'].apply(
...
if __name__ == '__main__':
    print(
        f"If we scraped new datas, then delete file {OUTPUT_FILE_NAME} and run this script")
    embedding()
