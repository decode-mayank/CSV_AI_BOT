...
df['tags'] = df['description'].str.findall(pattern, flags=re.IGNORECASE) + df['category'].str.findall(
    pattern, flags=re.IGNORECASE) + df['product'].str.findall(pattern, flags=re.IGNORECASE)
df['tags'] = df.apply(lambda x: x['breadcrumb'] + "|" +
                      x['product'] + "|" + "|".join(list(set(x['tags']))), axis=1)
